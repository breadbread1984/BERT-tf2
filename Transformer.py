...
def Attention(d_model, num_heads):
    ...
    # 1) inputs
    # NOTE: key_length == value_length
    query = tf.keras.Input((num_heads, None, d_model // num_heads)); # query.shape = (batch, heads, query_length, dimension)
    ...
    logits = tf.keras.layers.Lambda(lambda x: x[0] / tf.math.sqrt(x[1]) + x[2] * -1e9)([qk, depth, mask]); # logits.shape = (batch, heads, query_length, key_length)
    ...
    results = tf.keras.layers.Lambda(lambda x: tf.linalg.matmul(x[0], x[1]))([attention, value]); # results.shape = (batch, heads, query_length, dimension)
...
def MultiHeadAttention(d_model, num_heads):
    ...
    tf.debugging.Assert(tf.equal(d_model % num_heads,0),[d_model, num_heads]);
    # 1) inputs
    query = tf.keras.Input((None,d_model)); # query.shape = (batch, query_length, dimension)
    ...
    mask = tf.keras.Input((1, None, None)); # mask.shape = (batch, 1, query_length or 1, key_length)
    # 2) encoding
    query_dense = tf.keras.layers.Dense(units = d_model)(query); # query_dense.shape = (batch, query_length, dimension)
    ...
    query_splitted = tf.keras.layers.Lambda(lambda x: tf.transpose(x, (0, 2, 1, 3)))(query_splitted); # query_splitted.shape = (batch, heads, query_length, dimension // heads)
    ...
    attended = Attention(d_model, num_heads)([query_splitted, key_splitted, value_splitted, mask]);   # attended.shape = (batch, heads, query_length, dimension // heads)
    # 5) concat heads
    attended = tf.keras.layers.Lambda(lambda x: tf.transpose(x, (0, 2, 1, 3)))(attended); # attended.shape = (batch, query_length, heads, dimension // heads)
    concated = tf.keras.layers.Reshape((-1, d_model))(attended);                          # concated.shape = (batch, query_length, dimension)
    # 6) output
    results = tf.keras.layers.Dense(units = d_model)(concated);                           # results.shape = (batch, query_length, dimension)
    return tf.keras.Model(inputs = (query, key, value, mask), outputs = results);
...
def EncoderLayer(d_model, num_heads, code_dim, dropout_rate):
    ...
    tf.debugging.Assert(tf.equal(d_model % num_heads,0),[d_model, num_heads]);
    # 1) inputs
    inputs = tf.keras.Input((None, d_model));   # inputs.shape = (batch, input_length, dimension)
    mask = tf.keras.Input((1, 1, None));        # mask.shape = (batch, 1, 1(will be query_length), input_length)
    ...
    attended = tf.keras.layers.LayerNormalization(epsilon = 1e-6)(inputs_attended); # attended.shape = (batch, query_length, dimension)
    ...
    attended_outputs = tf.keras.layers.Add()([attended, outputs]);                  # attended_outputs.shape = (batch, query_length, dimension)
    # 4) outputs
    outputs = tf.keras.layers.LayerNormalization(epsilon = 1e-6)(attended_outputs); # outputs.shape = (batch, query_length, dimension)
...
def Encoder(vocab_size, num_layers, d_model, num_heads, code_dim, dropout_rate):
    ...
    outputs = tf.keras.layers.Dropout(rate = dropout_rate)(embeddings);  # embeddings.shape = (batch, input_length, dimension)
    # 3) multiple encode layers
    for i in range(num_layers):
        outputs = EncoderLayer(d_model, num_heads, code_dim, dropout_rate)([outputs, mask]); # outputs.shape = (batch, query_length, dimension)
    return tf.keras.Model(inputs = (inputs, mask), outputs = outputs);
...
def DecoderLayer(d_model, num_heads, code_dim, dropout_rate):
    ...
    # 1) inputs
    inputs = tf.keras.Input((None, d_model));          # inputs.shape = (batch, input_length, dimension)
    code = tf.keras.Input((None, d_model));            # code.shape = (batch, code_length, dimension)
    look_ahead_mask = tf.keras.Input((1, None, None)); # look_ahead_mask.shape = (batch, 1, input_length, code_length)
    padding_mask = tf.keras.Input((1, 1, None));       # padding_mask.shape = (batch, 1, 1(will be input_length), code_length)
    # 2) multi-head attention resblock
    ...
    attention1 = tf.keras.layers.LayerNormalization(epsilon = 1e-6)(attention1_inputs);     # attention1.shape = (batch, input_length, dimension)
    ...
    attention2 = tf.keras.layers.LayerNormalization(epsilon = 1e-6)(attention2_attention1); # attention2.shape = (batch, input_length, dimension)
    # 4) feed ward network
    outputs = tf.keras.layers.Dense(units = code_dim, activation = 'relu')(attention2);
    outputs = tf.keras.layers.Dense(units = d_model)(outputs);
    outputs = tf.keras.layers.Dropout(rate = dropout_rate)(outputs);
    outputs_attention2 = tf.keras.layers.Add()([outputs, attention2]);                      # outputs_attention2.shape = (batch, input_length, dimension)
    # 5) outputs.shape = (batch, seq_length, d_model)
    outputs = tf.keras.layers.LayerNormalization(epsilon = 1e-6)(outputs_attention2);       # outputs.shape = (batch, input_length, dimension)
...
def Decoder(vocab_size, num_layers, d_model, num_heads, code_dim, dropout_rate):
    ...
    inputs = tf.keras.Input((None,));                  # inputs.shape = (batch, input_length)
    code = tf.keras.Input((None, d_model));            # code.shape = (batch, code_length, dimension)
    look_ahead_mask = tf.keras.Input((1, None, None)); # look_ahead_mask.shape = (batch, 1, input_lenth, code_length)
    padding_mask = tf.keras.Input((1, 1, None));       # padding_mask.shape = (batch, 1, 1(will be input_length), code_length)
    # 2) token to positional embedding
    embeddings = tf.keras.layers.Embedding(vocab_size, d_model)(inputs);
    embeddings = tf.keras.layers.Lambda(lambda x, d_model: tf.math.sqrt(tf.cast(d_model, dtype = tf.float32)) * x, arguments = {'d_model': d_model})(embeddings);
    embeddings = PositionalEncoding(d_model)(embeddings);
    outputs = tf.keras.layers.Dropout(rate = dropout_rate)(embeddings); # outputs.shape = (batch, input_length, dimension)
    # 3) multiple decode layers
    for i in range(num_layers):
        outputs = DecoderLayer(d_model, num_heads, code_dim, dropout_rate)([outputs, code, look_ahead_mask, padding_mask]); # outputs.shape = (batch, input_length, dimension)
    return tf.keras.Model(inputs = (inputs, code, look_ahead_mask, padding_mask), outputs = outputs);
...
def Transformer(vocab_size, num_layers = 2, d_model = 256, num_heads = 8, code_dim = 512, dropout_rate = 0.1):
    ...
    # 2) generate code
    code = Encoder(vocab_size, num_layers, d_model, num_heads, code_dim, dropout_rate)([inputs, enc_padding_mask]); # code.shape = (batch, input_length, dimension)
    decoded = Decoder(vocab_size, num_heads, d_model, num_heads, code_dim, dropout_rate)([dec_inputs, code, look_ahead_mask, dec_padding_mask]); # decoded.shape = (batch, dec_input_length, dimension)
    # 3) output
    outputs = tf.keras.layers.Dense(units = vocab_size)(decoded); # outputs.shape = (batch, dec_input_length, vocab_size)
